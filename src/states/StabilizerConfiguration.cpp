...
void StabilizerConfiguration::start(mc_control::fsm::Controller & ctl)
{
  if(key_.empty())
  {
    mc_rtc::log::error_and_throw<std::runtime_error>("[{}] You must specify a \"key\" entry in the configuration",
                                                     name());
  }
  if(!ctl.datastore().has("StabilizerStandingState::getConfiguration"))
  {
    mc_rtc::log::error_and_throw<std::runtime_error>(
        "[{}] The StabilizerStandingState must be running before this state", name());
  }

  auto config = ctl.datastore().call<mc_rbdyn::lipm_stabilizer::StabilizerConfiguration>(
  ...
  ctl.datastore().call<void, const mc_rbdyn::lipm_stabilizer::StabilizerConfiguration &>(
      "StabilizerStandingState::setConfiguration", config);
}
